...
mod tests {
    ...
    fn prefers_the_executable_tree_when_it_is_inside_the_launch_directory() {
        ...
        let executable = Path::new("C:/workspace/seamint-rs/target/release/seamint.exe");
        assert_eq!(
            environment_search_start(current, executable),
            Path::new("C:/workspace/seamint-rs/target/release")
        );

        let external = Path::new("C:/test-bin/seamint.exe");
