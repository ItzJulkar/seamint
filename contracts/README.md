...
```powershell
.\target\release\seamint.exe deploy-executor
```
...
For independent verification, calculate `keccak256("seamint/SponsoredMintExecutor/v1" || sponsor)` as the salt and pass `salt || contracts/SponsoredMintExecutor.creation.hex` to the canonical factory. The expected address follows the EIP-1014 formula using that salt and the creation-code hash below.
